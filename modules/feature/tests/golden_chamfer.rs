//! Golden regression tests for the bracket-with-top-chamfer fixture (OCCT).

use opencad_feature::bracket_with_top_chamfer;
...
#[test]
fn golden_chamfer_default_volume_and_mass() {
...
#[test]
fn golden_chamfer_distance_patch_reduces_volume() {
    let fixture = load_fixture(FIXTURE);
    let default = fixture
        .cases
        .iter()
    ...
    let kernel = OcctGeometryKernel::new();
    let params = bracket_parameters();
    let mut default_model = bracket_with_top_chamfer().expect("model");
    ...
    let mut patched_model = bracket_with_top_chamfer().expect("model");
