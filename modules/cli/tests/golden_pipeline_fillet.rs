//! Golden pipeline tests for bracket-with-top-fillet (read → patch → regen).

use opencad_core::{DocumentId, DocumentMetadata};
use opencad_feature::{bracket_with_top_fillet, FeatureRegistry};
...
#[test]
fn golden_pipeline_fillet_default_regen_matches_fixture() {
    ...
    let doc_path = dir.path().join("bracket_fillet.ocad.d");
    write_fillet_fixture(&doc_path);
...
#[test]
fn golden_pipeline_fillet_radius_patch_reduces_volume() {
    let fixture = load_fixture(FIXTURE);
    let default = fixture
        .cases
        .iter()
