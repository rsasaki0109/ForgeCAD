...
| Render and desktop preview | Partial | `modules/render`, `modules/desktop`, reusable `run_desktop_smoke` with serializable summary, `modules/desktop/tests/desktop_smoke.rs`, `modules/desktop/tests/command_parity.rs`, Tauri shell under `apps/desktop`, unsigned `.github/workflows/desktop.yml` native matrix plus versioned bundle/checksum contract, and credential-gated `.github/workflows/desktop-signed-release.yml` with ADR-005 trust scope; local Windows MSI/NSIS build, checksum, release-binary smoke, and extracted-MSI smoke evidence is recorded in `docs/plans/desktop-release-evidence.md`; parameter edit/undo/redo use backend history and keep viewport state separate | Confirm the hosted native matrix, downloadable tagged artifacts, Linux AppImage smoke, and credentialed Authenticode/macOS verification (`MCAD-P1-001`, `MCAD-P1-002`, `MCAD-P1-004`) |
...
| Documentation and examples | Partial | Architecture, ADRs, API docs, examples, the P5-005 evidence guide, and this roadmap exist | Keep status/evidence synchronized in feature PRs (`MCAD-P0-002`, `MCAD-P0-003`) |
