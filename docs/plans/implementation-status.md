...
| Plugin API | Partial | `modules/plugin-api/src/manifest.rs` defines the serialized `musubicad.plugin-manifest.v1` manifest, explicit major/minor compatibility, data-only capabilities, and standard `OpenCadError` results; `registry.rs` validates schema/API/kind/capability policy and lists linked plugins in BTree ID order; feature/importer/exporter traits expose no document, kernel, UI, filesystem, or network handles; focused tests and `examples/plugin-example/manifest.json` cover the contract | Add validated CLI/Agent product integration (`MCAD-P4-003`) and expand compatibility/failure evidence (`MCAD-P4-004`) |
