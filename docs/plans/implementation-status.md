...
| Feature regeneration | Implemented | `modules/feature/src/` covers sketch, extrude, hole, revolve, fillet, chamfer, and patterns; `robot_joint_actuator_housing()` composes 22 deterministic nodes and `modules/feature/tests/occt_regen.rs` verifies its downstream shaft, bearing, fastener, rib, and mounting features across a tower edit | Preserve reference-stability coverage as feature families expand |
