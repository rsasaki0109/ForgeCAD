...
| Render and desktop preview | Partial | `modules/render`, `modules/desktop`, reusable `run_desktop_smoke` with serializable summary, `modules/desktop/tests/desktop_smoke.rs`, `modules/desktop/tests/command_parity.rs`, Tauri shell under `apps/desktop`, unsigned `.github/workflows/desktop.yml` native matrix plus versioned bundle/checksum contract, and credential-gated `.github/workflows/desktop-signed-release.yml` with ADR-005 trust scope; local Windows/Linux checks and hosted run `32612751044` prove all four native builds, versioned artifacts, checksums, and packaged Linux smoke as recorded in `docs/plans/desktop-release-evidence.md`; parameter edit/undo/redo use backend history and keep viewport state separate | Confirm downloadable tagged artifacts and credentialed Authenticode/macOS verification (`MCAD-P1-002`, `MCAD-P1-004`) |
