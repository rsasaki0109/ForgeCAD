...
| Plugin API | Partial | `modules/plugin-api` provides the versioned manifest, data-only capabilities, deterministic registry, and linked feature/importer/exporter invocation; `modules/cli/src/plugin.rs` builds a non-global host registry, validates patch results through the shared dry-run/transaction/history path, and keeps exporter persistence host-owned; `opencad plugin` and `opencad.plugin_list`/`opencad.plugin_invoke` have success and atomic-failure tests | Expand compatibility, failure-isolation, and standalone example-plugin evidence (`MCAD-P4-004`) |
