...
| Plugin API | Partial | `modules/plugin-api/src/manifest.rs` defines the serialized `musubicad.plugin-manifest.v1` manifest, explicit major/minor compatibility, serializable diagnostics, and standard `OpenCadError` results; `feature_plugin.rs`, `importer.rs`, and `exporter.rs` expose linked Rust request/result traits without document, kernel, UI, filesystem, or network handles; `modules/plugin-api` has focused serialization/compatibility tests and `examples/plugin-example/manifest.json` | Add deterministic registry/capability discovery and validated CLI/Agent product integration (`MCAD-P4-002`, `MCAD-P4-003`); expand compatibility/failure evidence (`MCAD-P4-004`) |
